...
mkdir build
cd build
${CMAKE_DIR:+${CMAKE_DIR}/bin/}cmake -DCMAKE_BUILD_TYPE=${AMREX_BUILD_TYPE} -DENABLE_PARTICLES=ON -DENABLE_ASSERTIONS=ON -DENABLE_FORTRAN=${AMREX_ENABLE_FORTRAN} -DENABLE_OMP=${AMREX_ENABLE_OPENMP} -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} ..
