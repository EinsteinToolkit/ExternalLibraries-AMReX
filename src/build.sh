...
if [ "$(echo ${AMREX_ENABLE_CUDA} | tr '[:upper:]' '[:lower:]')" = 'yes' ]; then
  AMREX_GPU_OPTIONS=(-DAMReX_CUDA=ON -DAMReX_GPU_BACKEND=CUDA -DAMReX_CUDA_ERROR_CAPTURE_THIS=ON -DAMReX_CUDA_ERROR_CROSS_EXECUTION_SPACE_CALL=ON ${CUCC:+"-DCMAKE_CUDA_COMPILER=${CUCC}"} ${CUCCFLAGS:+"-DCMAKE_CUDA_FLAGS=${CUCCFLAGS}"} -DCMAKE_CUDA_COMPILER_FORCED=ON -DCMAKE_CUDA_COMPILE_FEATURES=cuda_std_14 -DCMAKE_CUDA_IMPLICIT_INCLUDE_DIRECTORIES='${CMAKE_CXX_IMPLICIT_INCLUDE_DIRECTORIES}')
