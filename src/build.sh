...
if [ "$(echo ${AMREX_ENABLE_CUDA} | tr '[:upper:]' '[:lower:]')" = 'yes' ]; then
  AMREX_GPU_OPTIONS="-DENABLE_CUDA=ON -DAMReX_GPU_BACKEND=CUDA -DAMReX_CUDA_ERROR_CAPTURE_THIS=ON -DAMReX_CUDA_ERROR_CROSS_EXECUTION_SPACE_CALL=ON ${CUCC:+-DCMAKE_CUDA_COMPILER=${CUCC}}"
  #TODO: make possible to specify CUDA arch in case AMReX's auto-detect fails
  #  -DAMReX_CUDA_ARCH=75
else
  AMREX_GPU_OPTIONS="-DUSE_CUDA=OFF"
fi


mkdir build
cd build
${CMAKE_DIR:+${CMAKE_DIR}/bin/}cmake -DCMAKE_BUILD_TYPE=${AMREX_BUILD_TYPE} -DENABLE_PARTICLES=ON -DENABLE_ASSERTIONS=ON -DENABLE_FORTRAN=${AMREX_ENABLE_FORTRAN} ${AMREX_GPU_OPTIONS} -DENABLE_OMP=${AMREX_ENABLE_OPENMP} -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} ..
